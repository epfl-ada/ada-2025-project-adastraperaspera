...
# -----------------------------------------------------------
# 2. Compute WT estimated distances inside plaque bounding boxes
# -----------------------------------------------------------
...
def plot_decay(pid, tg_records, wt_records):
    ...
    sns.scatterplot(x=tg["tg_distances"], y=tg["tg_signatures"], s=6, alpha=0.35, label="Tg17")
    sns.scatterplot(x=wt["wt_distances"], y=wt["wt_signatures"], s=6, alpha=0.35, label="WT13")
...
# -----------------------------------------------------------
# 4. Compute plaque-wise decay statistics
# -----------------------------------------------------------
...
# -----------------------------------------------------------
# 5. Interpretation labels + confidence scoring
# -----------------------------------------------------------
...
def add_significance_and_labels(stats_df):
    ...
    def interpret(r):
        ...
        if not r["tg_sig"] and r["wt_sig"]:
            return "WT shows structure; likely misalignment or cortical layer difference."
        return "Neither Tg nor WT shows strong decay (weak plaque region or noisy area)."
