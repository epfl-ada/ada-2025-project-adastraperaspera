...
def plot_cell_to_plaque_map_visible(
    ...
    """Version that forces plaques to be clearly visible on top."""
    fig, ax = plt.subplots(figsize=figsize)

    # --- scatter cells first (so plaques overlay)
    norm = Normalize(
        vmin=np.percentile(cells_df[dist_col], 1),
        vmax=np.percentile(cells_df[dist_col], 99),
    )
    sc = ax.scatter(
        cells_df[x_col],
        cells_df[y_col],
    ...
    # --- now plot plaques clearly on top
    if not plaques_gdf.empty:
        plaques_gdf.plot(
            ax=ax,
            facecolor="none",  # transparent fill
            edgecolor="lime",  # bright green outline
            linewidth=1.2,
            label="Plaques",
            zorder=10,
        )
    ...
    ax.set_title("Cells and plaque outlines (visible check)")
